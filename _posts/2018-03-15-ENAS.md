...
2016년, 구글브레인에서 기발한 AutoML 방법론으로 NAS(Neural Architecture Search with Reinforcement Learning)라는 논문을 발표합니다. 네트워크 구조가 수도 없이 많이 나왔지만, 사실 데이터에 맞는 구조를 디자인하는것은 매우 힘든 작업입니다. 만들고 나서 튜닝하는 것 까지 합치면 굉장히 많은 노력이 필요합니다. 심지어 내가 만들어놓은 구조가 최고의 구조라는 보장조차도 없습니다. 
